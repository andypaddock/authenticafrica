<?php
/**
 * authenticafrica functions and definitions
 *
...
add_action('wp_enqueue_scripts', 'authenticafrica_scripts');

/* Add Menus */
add_action('init', 'authenticafrica_custom_menu');

/* Dashboard Config */
add_action('wp_dashboard_setup', 'authenticafrica_dashboard_widget');

/* Dashboard Style */
add_action('admin_head', 'authenticafrica_custom_fonts');

/* Remove Default Menu Items */
add_action('admin_menu', 'authenticafrica_remove_menus');

/* Change Posts Columns */
add_filter('manage_posts_columns', 'authenticafrica_manage_columns');

/* Reorder Admin Menu */
add_filter('custom_menu_order', 'authenticafrica_reorder_menu');
add_filter('menu_order', 'authenticafrica_reorder_menu');

/* Remove Comments Link */
add_action('wp_before_admin_bar_render', 'authenticafrica_manage_admin_bar');
...
function authenticafrica_scripts() {
	wp_enqueue_style( 'authenticafrica-style', get_template_directory_uri() . '/style.css', array(), filemtime(get_template_directory() . '/style.css'), false);
	wp_enqueue_script( 'authenticafrica-core-js', get_template_directory_uri() . '/inc/js/compiled.js', array('jquery'), filemtime( get_stylesheet_directory() . '/inc/js/compiled.js' ), true);
}

add_filter( 'script_loader_tag', function ( $tag, $handle ) {

	if ( 'authenticafrica-intersection-js' !== $handle )
		return $tag;

	return str_replace( ' src', ' defer="defer" src', $tag );
}, 10, 2 );

function authenticafrica_custom_menu() {
...
function authenticafrica_dashboard_widget() {
	global $wp_meta_boxes;
	wp_add_dashboard_widget('custom_help_widget', 'authenticafrica Support', 'authenticafrica_dashboard_help');
}

function authenticafrica_dashboard_help() {
	echo file_get_contents(__DIR__ . "/admin-settings/dashboard.html");
}

function authenticafrica_custom_fonts() {
...
function authenticafrica_remove_menus(){
	remove_menu_page( 'edit-comments.php' ); //Comments
}

function authenticafrica_manage_columns($columns) {
	unset($columns["comments"]);
	return $columns;
}

function authenticafrica_reorder_menu() {
    return array(
		'index.php',                        // Dashboard
		'separator1',                       // --Space--
		'edit.php',                         // Posts
...
function authenticafrica_manage_admin_bar(){
	global $wp_admin_bar;
